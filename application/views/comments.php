...
<head>
    ...
    <script type = "text/javascript">
      function message(){
        alert("Comment Successfully Changed");
      }
...
<div class = "main-grid">
  <div class = "agile-grids">
    <div class = "progressbar-heading grids-heading">
      <h2> <i class="fa fa-book"></i> Story</h2>
      <?php 
  ...
  <div class = "codes">
    <div class = "agile-container">
        ...
        $queryStories=$this->db->query("SELECT * FROM stories INNER JOIN users ON stories.userID = users.userID where stories.storyID={$storyID}");
        ...
        <div class="card">
              ...
              <br>
              <div class="react col-xl-1">
                
                <?php 
                $userID=$this->session->userdata('userID');
                $querylikes=$this->db->query("SELECT * FROM likes WHERE storyID='{$rowStory->storyID}'");
              ...
              <div class="comment">
                ...
                <div class="text-center">
                  <?=form_open("main/post_comment")?>
                    <?=form_hidden("date",date("y_m_d H:i:s"))?>
                    <input type="hidden" name="userID" value="<?=$rowinfo->userID?>">
                    <input type="hidden" name="storyID" value="<?=$storyID?>">
                    <input class="text-center comment-box" type="text" name="comment" value="" placeholder="comment">
          ...
          <?php 
          $post_comments = $this->db->query("SELECT * FROM users INNER JOIN comments ON users.userID = comments.userID where comments.storyID=$storyID");
          foreach ($post_comments->result() as $cm) {
            
           ?>
          <div class="card">
            <div class="text-right">
              <?php 
                if($cm->userID != $rowinfo->userID){
               ...
               ?>
                <a class="btn btn-warning" data-toggle="modal" data-target="#modal<?=$cm->commentID?>"  href="#modal<?=$cm->commentID?>"><i class="fa fa-edit" aria-hidden="true"></i></a>
            <a class="btn btn-danger" data-toggle="modal" data-target="#modalDelete<?=$cm->commentID?>"  href="#modalDelete<?=$cm->commentID?>" href="#"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
            <?php 
              }
             ?>
              </div>
            <h3 class="font-white"><?=$cm->Fname?> <?=$cm->Mi?> <?=$cm->Lname?></h3>
            <div class="card-body">
               <p class="font-grey"><?=$cm->comment?></p>
            </div>
          </div>
           <!--MODAL FOR EDIT-->
<div id="modal<?=$cm->commentID?>" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <center>
        <?=form_open("main/update_comment/$cm->commentID")?>
                    <?=form_hidden("date",date("y_m_d H:i:s"))?>
                    <input type="hidden" name="userID" value="<?=$rowinfo->userID?>">
                    <input type="hidden" name="storyID" value="<?=$storyID?>">
                    <input class="text-center comment-box" type="text" name="comment" value="<?=$cm->comment?>">
      </div>
...
<!--MODAL DELETE-->
<div id="modalDelete<?=$cm->commentID?>" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <center>
        <?=form_open("main/delete_comment/$cm->commentID")?>

            <p>Do you want to delete this Comment permanently?</p>
